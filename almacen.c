...
void menuMuestras()
{
	...
	do {
		...
		printf("\t\t[2] Utilidad total del inventario\n");
...
void menuBuscarDescripcion()
{
	...
	do {
		...
		if (existeProducto2(descripcionProducto, &producto)) {
			...
			// Se muestran los datos del producto
			printf("\n\tEmpresa que elabora/distribuye el producto: %s\n", producto);
...
void menuUtilidad()
{
	...
	if (numeroProductos == 0) {
		...
		printf(" -------------------------------------------------------------------------------------------------------------\n");
		printf("%8s\t%-12s%15s%15s%15s%15s%10s\n", "CÓDIGO", "NOMBRE", "DESCRIPCIÓN", "COSTO C. $", "COSTO V. $", "UNIDADES");
		printf(" -------------------------------------------------------------------------------------------------------------\n");
		...
		for (i = 0; i < numeroProductos; i++) {
			if (productos[i].codigo != VALOR_CENTINELA) {
				printf("%7d \t%-15s %-15s %-8s%15.1f%15.1f%8d\n", productos[i].codigo, productos[i].nombre, productos[i].descripcion, productos[i].costo, productos[i].precio, productos[i].cantidad);
...
void menuStockMinimo()
{
	...
	for (i = 0; i < numeroProductos; i++){
		if (productos[i].cantidad <= minimo) {
	 	    printf("\nEl producto:\n\n");
	 	    printf("%8s\t%-12s%15s%15s%15s%15s\n", "CÓDIGO", "NOMBRE", "DESCRIPCIÓN", "COSTO C. $", "COSTO V. $");
	 	    printf("%7d \t%-15s %-15s %-8s%15.1f%15.1f\n", productos[i].codigo, productos[i].nombre, productos[i].descripcion, productos[i].costo, productos[i].precio);
...
void tituloPrincipal()
{
	int i;
	printf("\n                                                                           \n");//
	printf("\t\t\t                Materias primas Local: 76-77\n");
	printf("\t\t\t                 Tus compras mas baratas!\n");
